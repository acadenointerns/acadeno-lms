...
async function updateLeadStatus(req, res) {
  ...
  try {
    ...
    const shouldLock = (new_status === 'converted');
...
async function getDashboard(req, res) {
  ...
  try {
    ...
    // 1. Redis Cache Implementation natively isolating logic execution (TTL=300s)
...
async function convertLead(req, res) {
  ...
  try {
    ...
    // Execute Conversion: Status=converted, is_locked=true
    const updateRes = await client.query(
      `UPDATE leads 
    ...
    await client.query(
      `INSERT INTO lead_status_history (lead_id, changed_by, from_status, to_status, reason) 
       VALUES ($1, $2, $3, 'converted', 'Lead converted to enrollment pipeline')`,
